...
import { updateStockItemType } from '../services/updateStockItemType';
...
export async function putStockItemTypeController(req: Request, res: Response) {
  const { companyId } = req;
  const {
    id: stockItemTypeId,
    name,
  ...
  if (existsStockItemType && existsStockItemType.id !== stockItemTypeId) {
  ...
  const stockItemType = await updateStockItemType<StockItemTypeOutput>({
    data: {
      select: {
        id: true,
        name: true,
  ...
  if (!stockItemType) {
    return res.status(500).json({
      ServerMessage: {
        statusCode: 500,
        message: 'Erro ao editar tipo de item.',
      },
    });
  }
  ...
  return res.status(201).json({
    ...
    ServerMessage: {
      statusCode: 201,
      message: `Tipo de item '${stockItemType.name}' editado com sucesso.`,
    },
  });
}
