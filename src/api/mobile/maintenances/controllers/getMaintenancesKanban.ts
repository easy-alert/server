...
import { findUserById } from '../../../client/user/services/findUserById';
...
export async function getMaintenancesKanban(req: Request, res: Response) {
  ...
  const buildingPermissions = req?.BuildingsPermissions;
  ...
  if (userId) {
    ...
    // buildingPermissions =
    //   userData?.UserBuildingsPermissions.map((building) => ({
    //     Building: {
  ...
  const permittedBuildingsIds = isAdmin
    ? undefined
    : handlePermittedBuildings(buildingPermissions, 'id');
  ...
  let companyIdFilter = Company?.id;
  ...
  const company = await findCompanyById({ companyId: companyIdFilter });
